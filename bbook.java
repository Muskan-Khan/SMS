...
public class bbook extends Frame implements ActionListener
{
	...
	JLabel lbl=new JLabel("Report on Booking");
	...
	public bbook()
	{
		...
		table2.setBounds(500,400,800,200);
		sp.setBounds(500,400,800,200); 	
		...
		if(e.getSource()==bprint)
		{
			try
			{
				PrintWriter objpr=new PrintWriter("bbook.txt");
				...
				con=DriverManager.getConnection("jdbc:odbc:sms");
				Statement st=con.createStatement();
					res=st.executeQuery("Select * from booking where book_date='"+txdate.getText()+"'");
					while(res.next())
					{
